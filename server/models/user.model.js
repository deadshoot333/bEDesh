...
async function findUserByMobile(mobile) {
  ...
  try {
    const result = await pool.query("SELECT * FROM public.users WHERE mobile_number = $1", [
      mobile,
    ]);
    console.log('📊 Query result - rows found:', result.rows.length);
    return result.rows[0];
...
async function createUser(email, mobile, hashedPassword, countryApplyingFrom, universityCountry, universityCity) {
  console.log('\n🗄️  DATABASE - createUser called');
  console.log('📝 Parameters:');
  console.log('  - email:', email);
  console.log('  - mobile:', mobile);
  console.log('  - hashedPassword:', hashedPassword ? '[REDACTED]' : 'undefined');
  ...
  try {
    ...
    const result = await pool.query(
      "INSERT INTO public.users (email, mobile_number, password, country_applying_from, university, city) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id, email",
      [email, mobile, hashedPassword, countryApplyingFrom, universityCountry, universityCity]
    );
...
async function findUserById(id) {
  ...
  try {
    const result = await pool.query("SELECT id, email FROM public.users WHERE id = $1", [
      id,
    ]);
    console.log('📊 Query result - rows found:', result.rows.length);
    return result.rows[0];
...
async function getToken(refreshToken) {
  ...
  try {
    const result = await pool.query("SELECT * FROM public.rf_tokens WHERE token = $1",[refreshToken])
...
async function insertToken(token,userId) {
  ...
  try {
    const result = await pool.query(
      "INSERT INTO public.rf_tokens (token, user_id) VALUES ($1, $2)", [token, userId]
    )
...
async function deleteToken(refreshToken) {
  ...
  try {
    await pool.query('DELETE FROM public.rf_tokens WHERE token = $1', [refreshToken]);
